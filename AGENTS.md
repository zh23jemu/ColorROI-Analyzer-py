...
- 用户完成公网测试后，终止 AWS 实例 `i-017b0027602f81aed` 并清理安全组 `sg-06d20438196dca6f6` 与 S3 部署包 `colorroi-deploy-553432479592-ap-east-1/colorroi-deploy.zip`，避免持续计费。
...
- AWS 公网测试实例当前开放 `80` 到 `0.0.0.0/0`，安全组中仍保留 `8501` 规则；仅适合短期用户验收。应用没有登录鉴权，不应上传敏感图片，测试结束后应尽快清理云资源。
