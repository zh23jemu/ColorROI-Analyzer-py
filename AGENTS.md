...
- 评估是否替换 `streamlit-drawable-canvas`：若用户要求橡皮擦和画笔一样即时、无刷新、且只擦黄色/红色标记层，需要开发自定义前端画布组件或改用支持多图层/前端 mask 的组件。
...
- 当前第三方画布组件只能即时显示新增笔画，不能在浏览器端直接修改后端累计 mask；橡皮擦对已有黄色/红色标记的最终效果依赖 Streamlit 回传后重绘背景，因此仍可能有轻微刷新感。
