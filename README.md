...
当前版本支持自动生成皮损/ROI 候选区域，也保留医生或用户手动画黄色 ROI 边界的校正入口。手动画出的 ROI 优先级最高；如果未手动画 ROI 且启用自动识别，系统会先生成皮损候选，再在该 ROI 内优先使用红色手动毛发/遮挡标注，如果没有手动标注，则自动检测毛发候选并从有效分析区中排除。
...
- `src/colorroi_analyzer/`：Python 核心算法包，包含图片读取、自动皮损候选、ROI 边界填充、毛发区域修复、Lab 转换、四分类聚类和 DMDI 计算。
...
当前项目建议使用 Python 3.11。本机已用 `.venv` 验证通过。依赖中已将 Streamlit 约束为 `>=1.35,<1.40`，并在应用中保留兼容补丁，避免 `streamlit-drawable-canvas` 在新版 Streamlit 中上传图片时报 `image_to_url` 缺失。
...
2. 可以直接点击分析使用自动皮损/ROI 候选；如自动候选不准，则在画布上使用黄色画笔手动画出皮损/ROI 边界，手动画结果会优先生效。
