...
- 按用户截图调整左侧控制栏顺序：先显示 Sample Information 和 Upload image，再显示临床毛发、Pattern、Drawing mode、Brush size、清空、Inpaint 和 Analyze；当前 `.venv\Scripts\python.exe -m pytest` 11 项通过，`import app` 正常。
- 按用户要求清理 AWS `ap-east-1` 公网测试资源：终止 `i-035e3e54eeef40e32`，系统盘 `vol-0044c68a59cb40607` 随实例删除，安全组 `sg-06d20438196dca6f6` 已删除，未发现 ColorROI 相关 EIP、负载均衡或 NAT Gateway。
- Saved Records 表格改为可勾选删除：新增 `Delete` 选择列和 `Delete selected records` 按钮，用户可删除误保存或重复保存的行，CSV 导出仍只包含正式分析字段；当前 `.venv\Scripts\python.exe -m pytest` 11 项通过，`import app` 正常。
