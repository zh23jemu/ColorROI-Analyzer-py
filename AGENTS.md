...
- 在 AWS `ap-east-1` 创建公网测试部署：使用已有 VPC 子网 `subnet-03781c1b2a13b3e3d`，安全组 `sg-06d20438196dca6f6` 开放 TCP `8501`，实例 `i-052190b6f08e3282b` 运行 Streamlit 服务，测试地址 `http://54.46.124.181:8501` 已返回 200。首次失败实例 `i-0bd30dba0e4760914` 已终止。
...
- 用户完成公网测试后，终止 AWS 实例 `i-052190b6f08e3282b` 并清理安全组 `sg-06d20438196dca6f6` 与 S3 部署包 `colorroi-deploy-553432479592-ap-east-1/colorroi-deploy.zip`，避免持续计费。
...
- AWS 公网测试实例当前开放 `8501` 到 `0.0.0.0/0`，仅适合短期用户验收；应用没有登录鉴权，不应上传敏感图片，测试结束后应尽快清理云资源。

## Architecture Decisions

- Python 重建采用核心算法包和 Streamlit UI 分离的结构，便于非交互式测试和后续界面维护。
- 原项目资源和测试产物保留为迁移参考资料，不作为 Python 运行入口；后续验收以 Python 版功能和用户确认效果为准。
- 临时公网验收优先采用单台 EC2 `t3.micro` 直接运行 Streamlit，减少部署复杂度和固定成本；生产化再考虑域名、HTTPS、鉴权、容器化或托管平台。
